...
<button data-page="dashboard" class="active"><span class="nav-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="7" height="7" rx="2"/><rect x="14" y="3" width="7" height="7" rx="2"/><rect x="3" y="14" width="7" height="7" rx="2"/><rect x="14" y="14" width="7" height="7" rx="2"/></svg></span><span class="nav-label">仪表盘</span></button>
...
let tok=sessionStorage.getItem('ig830Token')||'', user=sessionStorage.getItem('ig830User')||'', lastChannelConfig={}, lastRuntimeChannelErrors={}, lastRuntimeChannelTests={}, channelTestState={}, logPage=1, logPages=1; const LOG_PAGE_SIZE=20; const $=id=>document.getElementById(id);
...
async function refreshAll(){try{let [c,s,l,st]=await Promise.all([api('/api/config'),api('/api/status'),api('/api/logs'),api('/api/stats')]);fill(c);let r=s.runtime||{},errors=r.channel_errors||{},tests=r.channel_tests||{};updateChannelDots(c,errors,tests);let health=channelHealthSummary(c,errors,tests),status=!c.enabled?'已停用':health.active===0?'等待通道配置':health.healthy===health.active?'运行中':health.healthy>0?'部分通道异常':health.untested===health.active?'等待通道测试':'转发异常';$('dashboardForwardStatus').textContent=status;$('dashboardForwardStatus').className=status==='运行中'?'ok':status.includes('异常')?'bad':'warn';$('dashboardChannelCount').textContent=`${health.healthy} 正常 / ${health.active} 已启用`;$('dashboardChannelErrors').textContent=String(health.abnormal);$('dashboardChannelErrors').className=health.abnormal?'bad':'ok';$('dashboardLastPoll').textContent=localTime(r.last_poll);if($('appVersion'))$('appVersion').textContent=`v${s.version}`;if($('configSchema'))$('configSchema').textContent=String(s.config_schema);$('usb').textContent=s.usb_present?'已识别':'未识别';$('usb').className=s.usb_present?'ok':'bad';let usbModeLabel=s.usb_mode==='compatible'?'Linux 兼容':s.usb_mode==='factory'?'大疆出厂':'未知';$('usbMode').textContent=s.usb_present?`${usbModeLabel} · ${s.usb_id||'—'}`:'未检测到';$('usbMode').className=s.usb_present?'ok':'bad';$('serial').textContent=s.serial_present?s.serial_port:'不可用';$('serial').className=s.serial_present?'ok':'bad';$('sim').textContent=r.sim_ready?'已就绪':'未就绪';$('sim').className=r.sim_ready?'ok':'bad';$('phoneNumber').textContent=r.phone_number||'SIM 未提供';$('phoneNumber').className=r.phone_number?'ok':'warn';$('operator').textContent=r.operator||'未识别';$('operator').className=r.registered?'ok':'warn';$('registration').textContent=r.registration||'未知';$('registration').className=r.registered?'ok':'warn';let level=Number(r.signal_level||0);document.querySelectorAll('#signalBars i').forEach((e,i)=>e.classList.toggle('on',i<level));$('signalText').textContent=r.signal_dbm==null?'未知':`${r.signal_dbm} dBm · ${level}/5`;$('note').textContent=s.note+(r.last_error?'；'+r.last_error:'');$('logs').innerHTML=l.length?l.map(x=>`<div class="entry"><b>${esc(logKindLabel(x.kind))}</b> ${esc(localizedLogMessage(x.message))}<br><small>${esc(localTime(x.time))}</small></div>`).join(''):'<div class="sub">暂无记录</div>';$('statMessages').textContent=st.messages;$('statToday').textContent=st.today;$('statSuccess').textContent=st.deliveries.success||0;$('statFailed').textContent=(st.deliveries.failed||0)+(st.deliveries.pending||0);$('topSenders').textContent=st.top_senders.length?'常见发送方：'+st.top_senders.map(x=>`${x.sender}（${x.count} 条）`).join(' · '):'暂无发送方统计';$('copyrightYear').textContent=String(new Date().getFullYear());loadMessages()}catch(e){if(e.message.includes('401'))logout();else msg(e.message,true)}}
...
function showPage(page){document.querySelectorAll('.nav button').forEach(x=>x.classList.toggle('active',x.dataset.page===page));document.querySelectorAll('.grid>section').forEach(s=>s.classList.toggle('hidden',!s.dataset.page.split(' ').includes(page)));if(page==='proxy')showChannel(localStorage.getItem('nexrelayChannel')||'custom');let b=document.querySelector(`.nav button[data-page="${page}"]`);$('pageTitle').textContent=b?.querySelector('.nav-label')?.textContent||'控制台';localStorage.setItem('nexrelayPage',page);toggleSidebar(false);scrollTo(0,0);if(page==='sms')loadMessages();if(page==='logs')refreshAll()}
const showPageBase=showPage;showPage=function(page){showPageBase(page);if(page==='logs')loadLogs(logPage)};
document.querySelectorAll('.nav button').forEach(b=>b.onclick=()=>showPage(b.dataset.page));
showPage(localStorage.getItem('nexrelayPage')||'dashboard');
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/config":
            ...
            self.send_json(DB.list_messages(query.get("q",[""])[0], bounded_int(query.get("page",[1])[0],1,1,1000000)))
        elif path == "/api/stats":
