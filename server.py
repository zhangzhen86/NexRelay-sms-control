...
.time-sort{display:inline-flex;align-items:center;gap:6px;white-space:nowrap}.sort-button{display:inline-grid;place-items:center;width:28px;min-width:28px;height:28px;min-height:28px;padding:0;border:1px solid var(--line);border-radius:7px;background:transparent;color:var(--text);font-size:1rem;line-height:1}.sort-button:hover{background:#e8e8ed;color:#1d1d1f}html[data-theme="dark"] .sort-button:hover{background:#3a3a3c;color:#fff}
</style></head><body>
...
let tok=sessionStorage.getItem('ig830Token')||'', user=sessionStorage.getItem('ig830User')||'', lastChannelConfig={}, lastRuntimeChannelErrors={}, lastRuntimeChannelTests={}, channelTestState={}, logPage=1, logPages=1, messageSort='desc'; const LOG_PAGE_SIZE=20; const $=id=>document.getElementById(id);
...
async function loadMessages(){try{let q=encodeURIComponent($('messageSearch')?.value||'');let r=await api(`/api/messages?q=${q}&sort=${messageSort}`);$('messageRows').innerHTML=r.items.length?r.items.map(m=>`<tr><td>${esc(localTime(m.received_at||m.stored_at))}</td><td>${esc(m.sender)}</td><td>${esc(m.body)}</td><td>${m.deliveries.filter(d=>d.channel).map(d=>`<span class="badge ${d.status==='success'?'ok':d.status==='failed'?'bad':'warn'}">${esc(channelLabels[d.channel]||d.channel)} · ${esc(deliveryLabels[d.status]||d.status)}${d.status==='failed'?` <button type="button" class="retry-link" onclick="retryDelivery(${d.id})">重试</button>`:''}</span>`).join('')||'<span class="sub">尚未转发</span>'}</td></tr>`).join(''):'<tr><td colspan="4" class="sub">暂无短信</td></tr>'}catch(e){msg('收件箱加载失败：'+e.message,true)}}
function toggleMessageSort(){messageSort=messageSort==='desc'?'asc':'desc';let b=$('messageSortButton'),latest=messageSort==='desc';b.textContent=latest?'↓':'↑';b.title=latest?'当前最新在前，点击改为最早在前':'当前最早在前，点击改为最新在前';b.setAttribute('aria-label',latest?'当前最新收到的短信在顶部，点击切换为最早在顶部':'当前最早收到的短信在顶部，点击切换为最新在顶部');loadMessages()}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/config":
            ...
            self.send_json(DB.list_messages(query.get("q",[""])[0], bounded_int(query.get("page",[1])[0],1,1,1000000), order=order))
        elif path == "/api/stats":
